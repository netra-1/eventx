    ...
    if (localStorage.getItem('token') && localStorage.getItem('category')=="MANAGER"){
        ...
        >
          ...
          <div className="mt-4 flex flex-col gap-4 relative">
            ...
            <Link
                onClick={()=>{LogOut()}}
                className="group flex items-center text-sm text-red-600  gap-3.5 font-medium p-2 hover:bg-gray-800 rounded-md">
                <div>{React.createElement(MdLogout, { size: "20" , color:"red" })}</div>
                <h2
                  style={{
                    transitionDelay: `${10 + 3}00ms`,
                  }}
                  className={`whitespace-pre duration-500 ${
                    !open && "opacity-0 translate-x-28 overflow-hidden text-red-600"
                  }`}
                >
                  Logout
                </h2>
