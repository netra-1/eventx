...
//   };
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/cake/" + cakeId)
      .then((response) => {
  ...
  const handleClick = async (e) => {
    ...
    data.append("image", image);
      ...
      await axios
        .put(
          "http://localhost:8000/api/cake/" + cakeId,
          data
        )
        .then(() => {
      ...
      <div className="new">
        <div className="newContainer">
          <div className="top mt-5">
            <h1 className="text-center pb-4">Update Cake</h1>
          </div>
            ...
            <form class="w-full max-w-lg">
              <div class="flex flex-wrap -mx-3 -mb-6">
                <div class="w-full md:w1/2 px-3 md:mb-0">
                  ...
                  <input size="50" value={name} onChange={(e) => setName(e.target.value)} class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-300 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="grid-first-name" type="text" placeholder="Enter cake name" />
                </div>
              </div>
              <div class="flex flex-wrap -mx-3 -mb-6">
                <div class="w-full md:w1/2 px-3 md:mb-0">
                  ...
                  <textarea value={description} rows="3" cols="50" onChange={(e) => setDescription(e.target.value)} class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-300 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="grid-first-name" type="text" placeholder="Enter description" />
                </div>
              </div>
